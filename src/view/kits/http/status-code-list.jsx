...
export default function HTTPStatusCodeList() {
	useDocumentTitle('HTTP 协议状态码');
	return (
		<Stack sx={{ px: 1, py: 1.5 }}>
			{Object.entries(HTTP_STATUS).map(([group, data]) => (
				<List key={group} sx={{ p: 0 }} subheader={<GroupHeader label={data.label} desc={data.desc} />}>
...
function GroupHeader({ label, desc }) {
	return (
		<ListSubheader component="div" sx={{ pt: 1, borderBottom: '1px solid var(--border-color)' }}>
