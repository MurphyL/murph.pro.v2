...
export default function JSONPathQuery() {
    ...
    const { height } = useComponentSize(queryRef);
    ...
    return (
        <Splitter sizes={[40, 60]} minSize={[500, 600]}>
            ...
            <Box sx={{ p: 1, pb: 2, width: '100%' }}>
                ...
                <Group title={`${state.evaluatorName} - 查询结果`} sx={{ height: `calc(100% - ${height}px - 26px)`, overflowY: 'auto' }}>
                    {state.message ? (
                        <Alert severity={state.message.severity || 'warn'} sx={{ mx: 0.5, my: 2 }}>{state.message.text}</Alert>
                    ) : null}
                    {state.result ? (
                        <React.Fragment>
                            {state.selected.includes('tree') ? (
                            ...
                            {state.selected.includes('table') && Array.isArray(state.data) ? (
                                <TableInspector data={state.data} theme={{ ...chromeLight, BASE_LINE_HEIGHT: '30px' }} />
                            ) : null}
                            {state.selected.includes('code') ? (
                                <CodeBlock language="json" dark={false} children={state.result} />
                            ) : null}
                        </React.Fragment>
                    ) : null}
                </Group>
